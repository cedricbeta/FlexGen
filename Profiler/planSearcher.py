...
GB = 1024**3  # Define a GB constant for convenience
MB = 1024**2  # Define a MB constant for convenience
BIT_TO_BYTE = 1 / 8  # Conversion factor from bits to bytes
...
def report_system_status(drive_name="/dev/sda2"):
    ...
    GPUs = GPUtil.getGPUs()
    status['gpu_memory_total'] = sum(gpu.memoryTotal for gpu in GPUs) if GPUs else 0
    ...
    status['disk_read_speed'] = disk_speed.get_read_speed()  # bits/s
    status['disk_write_speed'] = disk_speed.get_write_speed()  # bits/s

    return status

def update_cost_model_config_from_system_status(config_instance, drive_name="/dev/sda2"):
    system_status = report_system_status(drive_name)

    # Update the CostModelConfig instance
    config_instance.cmem = system_status['ram_total']
    config_instance.gmem = system_status['gpu_memory_total'] * GB
    config_instance.nmem = system_status['disk_total']
    config_instance.dtoc_bdw = system_status['disk_read_speed'] * BIT_TO_BYTE / MB  # Convert bits/s to MB/s
    config_instance.ctod_bdw_cache_p = system_status['disk_write_speed'] * BIT_TO_BYTE / MB  # Convert bits/s to MB/s
...
if __name__ == "__main__":
    ...
    config = update_cost_model_config_from_system_status(config, args.drive)
    print(config)
